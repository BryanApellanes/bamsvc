...
public class BamsvcConfigurationShould : UnitTestMenuContainer
{
    ...
    public void ParseServerNameFromPositionalArg()
    {
        ...
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(new[] { "my-svc" });
    ...
    public void DefaultServerNameToBamsvc()
    {
        ...
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(Array.Empty<string>());
    ...
    public void ParseHttpPort()
    {
        ...
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(new[] { "--port=9090" });
    ...
    public void EnableHttpsByFlag()
    {
        ...
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(new[] { "--https" });
    ...
    public void ParseHttpsPort()
    {
        ...
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(new[] { "--https", "--https-port=8444" });
    ...
    public void EnableTcpAndUdpWithCombinedFlag()
    {
        When.A<BamsvcConfiguration>(
            ...
            (config) => new { config.EnableTcp, config.EnableUdp })
        .TheTest
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(new[] { "--tcp-udp" });
    ...
    public void EnableTcpIndependently()
    {
        ...
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(new[] { "--tcp" });
            because.ItsTrue("TCP is enabled", config.EnableTcp);
    ...
    public void EnableUdpIndependently()
    {
        ...
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(new[] { "--udp" });
    ...
    public void ParseTcpAndUdpPorts()
    {
        When.A<BamsvcConfiguration>(
            ...
            (config) => new { config.TcpPort, config.UdpPort })
        .TheTest
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(new[] { "--tcp-port=9413", "--udp-port=9414" });
    ...
    public void ParseCertificatePath()
    {
        ...
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(new[] { "--cert=/path/to/cert.pfx" });
    ...
    public void ParseFullConfiguration()
    {
        ...
        .ShouldPass(because =>
        {
            var config = BamsvcConfiguration.FromArgs(fullArgs);
